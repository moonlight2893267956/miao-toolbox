...
public class GitHubOAuthService {
    ...
    public LoginResponse handleCallback(String code, String state, HttpServletResponse response) {
        ...
        try {
            user = userRepository.findByGithubId(String.valueOf(githubUser.getId()))
                    .orElseGet(() -> createOAuthUser(githubUser, inviteToken));
    ...
    private User createOAuthUser(GitHubUser githubUser, String inviteToken) {
        ...
        try {
            ...
            log.warn("GitHub OAuth register with invalid invite token, falling back to USER role: {}", e.getMessage());
