...
NACOS_ADDR="${NACOS_ADDR:?NACOS_ADDR 未设置（请在 .env 配置，例如 nacos.yunmiao.site:39848）}"
...
curl -s -o /dev/null -w "namespace create: %{http_code}\n" -X POST \
...
push() {
  ...
  curl -s -X POST "${BASE}/v1/cs/configs?accessToken=${TOKEN}" \
